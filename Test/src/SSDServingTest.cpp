...
template <typename T>
void GenerateVectors(std::string fileName, SPTAG::SizeType rows, SPTAG::DimensionType dims,
                     SPTAG::VectorFileType fileType)
{
...
void GenVec(std::string vectorsName, SPTAG::VectorValueType vecType, SPTAG::VectorFileType vecFileType,
            SPTAG::SizeType rows = 1000, SPTAG::DimensionType dims = 100)
{
    switch (vecType)
    {
...
std::string CreateBaseConfig(SPTAG::VectorValueType p_valueType, SPTAG::DistCalcMethod p_distCalcMethod,
                             SPTAG::IndexAlgoType p_indexAlgoType, SPTAG::DimensionType p_dim, std::string p_vectorPath,
                             SPTAG::VectorFileType p_vectorType, SPTAG::SizeType p_vectorSize,
                             std::string p_vectorDelimiter, std::string p_queryPath, SPTAG::VectorFileType p_queryType,
                             SPTAG::SizeType p_querySize, std::string p_queryDelimiter, std::string p_warmupPath,
                             SPTAG::VectorFileType p_warmupType, SPTAG::SizeType p_warmupSize,
                             std::string p_warmupDelimiter, std::string p_truthPath, SPTAG::TruthFileType p_truthType,
                             bool p_generateTruth, std::string p_headIDFile, std::string p_headVectorsFile,
                             std::string p_headIndexFolder, std::string p_ssdIndex)
{
...
void TestHead(std::string configName, std::string OutputIDFile, std::string OutputVectorFile, std::string baseConfig)
{
    ...
    config << "Ratio="
           << "0.2" << std::endl;
    config << "RecursiveCheckSmallCluster="
           << "true" << std::endl;
    config << "PrintSizeCount="
           << "true" << std::endl;
...
void TestBuildHead(std::string configName, std::string p_headVectorFile, std::string p_headIndexFile,
                   SPTAG::IndexAlgoType p_indexAlgoType, std::string p_builderFile, std::string baseConfig)
{
...
void TestBuildSSDIndex(std::string configName, std::string p_vectorIDTranslate, std::string p_headIndexFolder,
                       std::string p_headConfig, std::string p_ssdIndex, bool p_outputEmptyReplicaID,
...
void TestSearchSSDIndex(std::string configName, std::string p_vectorIDTranslate, std::string p_headIndexFolder,
                        std::string p_headConfig, std::string p_ssdIndex, std::string p_searchResult,
                        std::string p_logFile, std::string baseConfig)
{
    std::ofstream config(configName);
    if (!config.is_open())
    {
...
void RunFromMap()
{
    ...
    SPTAG::SSDServing::BootProgram(true, &myMap, nullptr, SPTAG::VectorValueType::Int8, SPTAG::DistCalcMethod::L2,
                                   dataFilePath.c_str(), indexFilePath.c_str());

    std::ostringstream config;
    config << "[Base]" << std::endl;
    config << "ValueType="
           << "Int8" << std::endl;
    config << "DistCalcMethod="
           << "L2" << std::endl;
    config << "IndexAlgoType="
           << "KDT" << std::endl;
    config << "Dim=" << 100 << std::endl;
    config << "VectorPath="
           << "sddtest/vectors_Int8_DEFAULT.bin" << std::endl;
    config << "QueryPath="
           << "sddtest/vectors_Int8_DEFAULT.query" << std::endl;
    config << "QueryType="
           << "DEFAULT" << std::endl;
    config << "TruthPath="
           << "sddtest/vectors_Int8_L2_DEFAULT_DEFAULT.truth" << std::endl;
    config << "TruthType="
           << "DEFAULT" << std::endl;
    config << "IndexDirectory="
           << "zbtest" << std::endl;
    config << "GenerateTruth="
           << "true" << std::endl;
    config << std::endl;

    TestSearchSSDIndex("run_from_map_search_config.ini", "", "", "", "", "", "", config.str());
}
...
#define BUILD_HEAD_BUILDER_CONFIG(VT, DM, ALGO)                                                                        \
    SSDTEST_DIRECTORY "test_build_head_" #VT "_" #DM "_" #ALGO ".builder.ini"
...
#define SEARCH_SSD_BUILDER_CONFIG(VT, DM, ALGO)                                                                        \
    SSDTEST_DIRECTORY "test_search_ssd_" #VT "_" #DM "_" #ALGO ".builder.ini"
#define SEARCH_SSD_RESULT(VT, DM, ALGO, FT, TFT)                                                                       \
    SSDTEST_DIRECTORY "test_search_ssd_" #VT "_" #DM "_" #ALGO "_" #FT "_" #TFT ".result"
...
#define WTEV(VT, DM, FT)                                                                                               \
    ...
    {                                                                                                                  \
        ...
        std::string base_config =                                                                                      \
            CreateBaseConfig(SPTAG::VectorValueType::VT, SPTAG::DistCalcMethod::DM, SPTAG::IndexAlgoType::Undefined,   \
                             VECTOR_DIM, VECTORS(VT, FT), SPTAG::VectorFileType::FT, VECTOR_NUM, "", "",               \
                             SPTAG::VectorFileType::Undefined, -1, "", "", SPTAG::VectorFileType::Undefined, -1, "",   \
                             "", SPTAG::TruthFileType::Undefined, false, OutputIDFile, OutputVectorFile, "", "");      \
...
#define BDHD(VT, DM, ALGO, FT)                                                                                         \
    ...
    {                                                                                                                  \
        ...
        std::string base_config = CreateBaseConfig(                                                                    \
            SPTAG::VectorValueType::VT, SPTAG::DistCalcMethod::DM, SPTAG::IndexAlgoType::ALGO, VECTOR_DIM,             \
            VECTORS(VT, FT), SPTAG::VectorFileType::FT, VECTOR_NUM, "", "", SPTAG::VectorFileType::Undefined, -1, "",  \
            "", SPTAG::VectorFileType::Undefined, -1, "", "", SPTAG::TruthFileType::Undefined, false, "",              \
            HEAD_VECTORS(VT, DM, FT), HEAD_INDEX(VT, DM, ALGO, FT), "");                                               \
        TestBuildHead(configName, HEAD_VECTORS(VT, DM, FT), HEAD_INDEX(VT, DM, ALGO, FT), SPTAG::IndexAlgoType::ALGO,  \
                      builderFile, base_config);                                                                       \
    }
...
#define BDSSD(VT, DM, ALGO, FT)                                                                                        \
    ...
    {                                                                                                                  \
        ...
        std::string base_config = CreateBaseConfig(                                                                    \
            ...
            VECTORS(VT, FT), SPTAG::VectorFileType::FT, VECTOR_NUM, "", "", SPTAG::VectorFileType::Undefined, -1, "",  \
            "", SPTAG::VectorFileType::Undefined, -1, "", "", SPTAG::TruthFileType::Undefined, false,                  \
            HEAD_IDS(VT, DM, FT), "", HEAD_INDEX(VT, DM, ALGO, FT), SSD_INDEX(VT, DM, ALGO, FT));                      \
        TestBuildSSDIndex(configName, HEAD_IDS(VT, DM, FT), HEAD_INDEX(VT, DM, ALGO, FT),                              \
                          BUILD_SSD_BUILDER_CONFIG(VT, DM, ALGO), SSD_INDEX(VT, DM, ALGO, FT), true, base_config);     \
    }
...
#define SCSSD(VT, DM, ALGO, FT, TFT)                                                                                   \
    ...
    {                                                                                                                  \
        ...
        std::string base_config =                                                                                      \
            CreateBaseConfig(SPTAG::VectorValueType::VT, SPTAG::DistCalcMethod::DM, SPTAG::IndexAlgoType::ALGO,        \
                             VECTOR_DIM, VECTORS(VT, FT), SPTAG::VectorFileType::FT, VECTOR_NUM, "", QUERIES(VT, FT),  \
                             SPTAG::VectorFileType::FT, QUERY_NUM, "", QUERIES(VT, FT), SPTAG::VectorFileType::FT,     \
                             QUERY_NUM, "", TRUTHSET(VT, DM, FT, TFT), SPTAG::TruthFileType::TFT, true,                \
                             HEAD_IDS(VT, DM, FT), "", HEAD_INDEX(VT, DM, ALGO, FT), SSD_INDEX(VT, DM, ALGO, FT));     \
        TestSearchSSDIndex(configName, HEAD_IDS(VT, DM, FT), HEAD_INDEX(VT, DM, ALGO, FT),                             \
                           SEARCH_SSD_BUILDER_CONFIG(VT, DM, ALGO), SSD_INDEX(VT, DM, ALGO, FT),                       \
                           SEARCH_SSD_RESULT(VT, DM, ALGO, FT, TFT), "", base_config);                                 \
    }
