...
template <typename T>
void GenerateVectorDataSet(SPTAG::IndexAlgoType algo, std::string distCalcMethod, unsigned int id,
                           std::vector<T> &query, std::vector<T> &query_id, std::string File_name)
{
    ...
    std::shared_ptr<SPTAG::MetadataSet> metaset(
        new SPTAG::MemMetadataSet(n * sizeof(std::uint64_t), n * sizeof(std::uint64_t), n));
    for (auto x : meta)
    {
...
template <typename T> void TestMultiIndexScanN(SPTAG::IndexAlgoType algo, std::string distCalcMethod, unsigned int n)
{
