...
Server::Server(const std::string &p_address, const std::string &p_port, const PacketHandlerMapPtr &p_handlerMap,
               std::size_t p_threadNum)
    : m_requestHandlerMap(p_handlerMap), m_connectionManager(new ConnectionManager), m_acceptor(m_ioContext)
{
    ...
    if (errCode)
    {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Failed to resolve %s %s, error: %s", p_address.c_str(),
    ...
    if (errCode)
    {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Failed to bind %s %s, error: %s", p_address.c_str(), p_port.c_str(),
    ...
    if (errCode)
    {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Failed to listen %s %s, error: %s", p_address.c_str(), p_port.c_str(),
                     errCode.message().c_str());
...
void Server::SetEventOnConnectionClose(std::function<void(ConnectionID)> p_event)
{
...
{
    m_acceptor.async_accept([this](boost::system::error_code p_ec, boost::asio::ip::tcp::socket p_socket) {
        ...
        if (!p_ec)
        {
            m_connectionManager->AddConnection(std::move(p_socket), m_requestHandlerMap, 0);
        }
...
void Server::SendPacket(ConnectionID p_connection, Packet p_packet, std::function<void(bool)> p_callback)
{
