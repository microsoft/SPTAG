...
PacketHeader::PacketHeader()
    : m_packetType(PacketType::Undefined), m_processStatus(PacketProcessStatus::Ok), m_bodyLength(0),
      m_connectionID(c_invalidConnectionID), m_resourceID(c_invalidResourceID)
{
}

PacketHeader::PacketHeader(PacketHeader &&p_right)
    : m_packetType(std::move(p_right.m_packetType)), m_processStatus(std::move(p_right.m_processStatus)),
      m_bodyLength(std::move(p_right.m_bodyLength)), m_connectionID(std::move(p_right.m_connectionID)),
      m_resourceID(std::move(p_right.m_resourceID))
{
}

PacketHeader::PacketHeader(const PacketHeader &p_right)
    : m_packetType(p_right.m_packetType), m_processStatus(p_right.m_processStatus), m_bodyLength(p_right.m_bodyLength),
      m_connectionID(p_right.m_connectionID), m_resourceID(p_right.m_resourceID)
{
}
...
Packet::Packet(Packet &&p_right)
    : m_header(std::move(p_right.m_header)), m_buffer(std::move(p_right.m_buffer)),
      m_bufferCapacity(std::move(p_right.m_bufferCapacity))
{
}

Packet::Packet(const Packet &p_right)
    : m_header(p_right.m_header), m_buffer(p_right.m_buffer), m_bufferCapacity(p_right.m_bufferCapacity)
{
}
...
std::uint8_t *Packet::HeaderBuffer() const
{
    return m_buffer.get();
}
...
std::uint8_t *Packet::Buffer() const
{
    return m_buffer.get();
}
...
std::uint32_t Packet::BufferCapacity() const
{
