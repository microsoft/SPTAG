...
#include "inc/Core/Common/OptimizedDataset.h"
#include <chrono>
...
#define Search(CheckDeleted, CheckDuplicated) \
        ...
        m_pTrees->InitSearchTrees(m_pSamples.get(), m_fComputeDistance, p_query, p_space); \
        m_pTrees->SearchTrees(m_pSamples.get(), m_fComputeDistance, p_query, p_space, m_iNumberOfInitialDynamicPivots); \
        ...
        while (!p_space.m_NGQueue.empty()) { \
            ...
            for (DimensionType i = 0; i <= checkPos; i++) { \
                _mm_prefetch((const char *)(m_pSamples->At(node[i])), _MM_HINT_T0); \
            } \
            ...
            for (DimensionType i = 0; i <= checkPos; i++) { \
                ...
                float distance2leaf = m_fComputeDistance(p_query.GetQuantizedTarget(), m_pSamples->At(nn_index), GetFeatureDim()); \
            ...
            if (p_space.m_NGQueue.Top().distance > p_space.m_SPTQueue.Top().distance) { \
                m_pTrees->SearchTrees(m_pSamples.get(), m_fComputeDistance, p_query, p_space, m_iNumberOfOtherDynamicPivots + p_space.m_iNumberOfCheckedLeaves); \
            } \
        } \
        p_query.SortResult(); \
*/
...
#define Search(CheckDeleted, CheckDuplicated) \
        ...
        ErrorCode Index<T>::SearchTree(QueryResult& p_query) const
        {
            ...
            m_pTrees->InitSearchTrees(m_pSamples.get(), m_fComputeDistance, *p_results, *workSpace);
            m_pTrees->SearchTrees(m_pSamples.get(), m_fComputeDistance, *p_results, *workSpace, m_iNumberOfInitialDynamicPivots);
...
#pragma omp parallel for
                for (SizeType i = 0; i < GetNumSamples(); i++) {
                    COMMON::Utils::Normalize(m_pSamples->At(i), GetFeatureDim(), base);
                }
            }
            ...
            m_pTrees->BuildTrees<T>(m_pSamples.get(), m_iDistCalcMethod, m_iNumberOfThreads);
...
#undef DefineBKTParameter
            ...
            if (m_bOptDataset) {
                SizeType optcols = (SizeType)(sizeof(T) * m_pSamples->C() + sizeof(SizeType) * m_pGraph->m_iNeighborhoodSize);
                char* data = (char*)_mm_malloc(((size_t)optcols) * newR, ALIGN_SPTAG);
                ptr->m_pSamples.reset(new COMMON::OptimizedDataset<T>(newR, optcols, m_iDataBlockSize, m_iDataCapacity, data, true, 0, sizeof(T) * m_pSamples->C(), m_pSamples->Name()));
                ((ptr->m_pGraph)->GetData()).reset(new COMMON::OptimizedDataset<SizeType>(newR, optcols, m_iDataBlockSize, m_iDataCapacity, data, false, sizeof(T) * m_pSamples->C(), optcols, m_pGraph->Type()));
            }
            else {
                (ptr->m_pSamples)->Initialize(newR, m_pSamples->C(), m_iDataBlockSize, m_iDataCapacity);
                (ptr->m_pGraph)->GetData()->Initialize(newR, m_pGraph->m_iNeighborhoodSize, m_iDataBlockSize, m_iDataCapacity);
            }
            ...
            ptr->m_pTrees->BuildTrees<T>(ptr->m_pSamples.get(), ptr->m_iDistCalcMethod, omp_get_num_threads());
        ...
        ErrorCode Index<T>::RefineIndex(const std::vector<std::shared_ptr<Helper::DiskPriorityIO>>& p_indexStreams, IAbortOperation* p_abort)
        {
...

            {
                ...
                newTrees.BuildTrees<T>(m_pSamples.get(), m_iDistCalcMethod, omp_get_num_threads(), &indices, &reverseIndices);
                ...
                std::shared_ptr<COMMON::NeighborhoodGraph> newGraph;
                if ((ret = m_pGraph->RefineGraph<T>(this, indices, reverseIndices, p_indexStreams[2], newGraph, &(newTrees.GetSampleMap()))) != ErrorCode::Success) return ret;
            }
...
#pragma omp parallel for schedule(dynamic)
        ...
        ErrorCode Index<T>::AddIndex(const void* p_data, SizeType p_vectorNum, DimensionType p_dimension, std::shared_ptr<MetadataSet> p_metadataSet, bool p_withMetaIndex, bool p_normalized)
        {
            ...
            ErrorCode ret;
            {
                ...
                if (DistCalcMethod::Cosine == m_iDistCalcMethod && !p_normalized)
                {
                    ...
                    for (SizeType i = begin; i < end; i++) {
                        COMMON::Utils::Normalize((T*)(m_pSamples->At(i)), GetFeatureDim(), base);
                    }
                }
            ...
            if (end - m_pTrees->sizePerTree() >= m_addCountForRebuild && m_threadPool.jobsize() == 0) {
                m_threadPool.add(new RebuildJob(m_pSamples.get(), m_pTrees.get(), m_pGraph.get(), m_iDistCalcMethod));
            }
