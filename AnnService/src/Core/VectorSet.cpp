...
BasicVectorSet::BasicVectorSet(const ByteArray &p_bytesArray, VectorValueType p_valueType, DimensionType p_dimension,
                               SizeType p_vectorCount)
    : m_data(p_bytesArray), m_valueType(p_valueType), m_dimension(p_dimension), m_vectorCount(p_vectorCount),
...
VectorValueType BasicVectorSet::GetValueType() const
{
    return m_valueType;
}
...
DimensionType BasicVectorSet::Dimension() const
{
    return m_dimension;
}

SizeType BasicVectorSet::Count() const
{
    return m_vectorCount;
}

bool BasicVectorSet::Available() const
{
...
ErrorCode BasicVectorSet::AppendSave(const std::string &p_vectorFile) const
{
    ...
    // Update count header
    {
        ...
        if (fp_write == nullptr ||
            !fp_write->Initialize(p_vectorFile.c_str(), std::ios::binary | std::ios::out | (append ? std::ios::in : 0)))
            return ErrorCode::FailedOpenFile;
...
#define DefineVectorValueType(Name, Type)                                                                              \
    case SPTAG::VectorValueType::Name:                                                                                 \
        SPTAG::COMMON::Utils::BatchNormalize<Type>(reinterpret_cast<Type *>(m_data.Data()), m_vectorCount,             \
