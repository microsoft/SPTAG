        ...
        workers.emplace_back([&]() {
            ...
            for (;;) {
                ...
                for (SPTAG::SizeType j = 0; j < p_base->Count(); j++)
                {
                    ...
                    queryResult.AddPoint(j, p_quantizer->L2Distance(queryCode.data(), code));
                }
                queryResult.SortResult();

                int matches = 0;
                for (int j = 0; j < p_resultCount; ++j) {
                    for (int i = 0; i < p_resultCount; ++i) {
                        if (p_truth[query][j] == queryResult.GetResult(i)->VID) {
                            matches++;
                            break;
                        }
                    }
...
template <typename T>
ErrorCode RaBitQAutoTuner<T>::Run(std::shared_ptr<VectorSet>& p_base,
                               ...
                               RaBitQAutoTuneResult& p_result)
{
    ...
    std::shared_ptr<VectorSet> queries = std::make_shared<BasicVectorSet>(ByteArray((std::uint8_t*)(p_base->GetData()), (uint8_t*)p_base->GetVector(p_queryNum) - (uint8_t*)p_base->GetData(), false), 
                                                                          p_base->GetValueType(), p_base->Dimension(), p_queryNum);
    std::vector<std::vector<SizeType>> truth;
    TruthSet::GenerateTruth<T>(queries, p_base, "", p_distance, p_resultCount, TruthFileType::DEFAULT, nullptr, p_threads, &truth);
    ...
    status = SelectMinimumBits(
        ...
        [&](int p_bits, float& p_recall) {
            ...
            if (!candidate) {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Failed to create RaBitQ candidate from shared centroid!\n");
                return ErrorCode::Fail;
            }
            const ErrorCode evaluation = EvaluateBits<T>(
    ...
    if (!direxists(p_outputFolder.c_str())) mkdir(p_outputFolder.c_str());

    std::string modelFinal = p_outputFolder + FolderSep + "rabitq_auto_quantizer.bin";
    auto modelOutput = f_createIO();
    if (!modelOutput ||
        !modelOutput->Initialize(
            modelFinal.c_str(), std::ios::out | std::ios::binary) ||
    ...
    p_result.quantizerPath = modelFinal;    
    p_result.quantizer = selected;
    SPTAGLIB_LOG(
        Helper::LogLevel::LL_Info,
        "RaBitQ auto-tuning selected %d bits (Recall@%d=%.6f); encoded vectors at width %d\n",
        p_result.selectedBits, p_resultCount, p_result.recall, p_result.codeDimension);
    return ErrorCode::Success;
}

template <typename T>
ErrorCode RaBitQAutoTuner<T>::SelectMinimumBits(float p_targetRecall,
...
#define DefineVectorValueType(Name, Type) template class SPTAG::COMMON::RaBitQAutoTuner<Type>;
