...
MultiIndexScan::MultiIndexScan(std::vector<std::shared_ptr<VectorIndex>> vectorIndices, std::vector<void *> p_targets,
                               unsigned int k, float (*rankFunction)(std::vector<float>), bool useTimer,
                               int termCondVal, int searchLimit)
{
    // copy parameters
...
void MultiIndexScan::Init(std::vector<std::shared_ptr<VectorIndex>> vectorIndices, std::vector<ByteArray> p_targets,
                          std::vector<float> weight, unsigned int k, bool useTimer, int termCondVal, int searchLimit)
{
        ...
        for (int i = 0; i < numCols; i++)
        {
            ...
            if (seenSet.find(vid) == seenSet.end())
            {
                ...
                else
                {
                    consecutive_drops = 0;
                    if (pq.size() == k)
                        pq.pop(); // the largest score is squeezed out by the current score
