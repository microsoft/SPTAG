...
namespace SPTAG
{
    namespace SPANN
    {
        ...
        {
            m_queryResult->Reset();
            m_index->SearchIndexIterativeNext(*m_headQueryResult, *m_queryResult, m_headWorkspace, m_extraWorkspace,
m_isFirstResult); m_isFirstResult = false; if (m_queryResult->GetResult(0) == nullptr ||
m_queryResult->GetResult(0)->VID < 0)
            {
                return false;
            }
