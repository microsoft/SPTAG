...
DynamicNeighbors::DynamicNeighbors(const int *p_data, const int p_length) : c_data(p_data), c_length(p_length)
{
}
...
int DynamicNeighbors::operator[](const int p_id) const
{
...
int DynamicNeighbors::Size() const
{
    return c_length;
}
...
DynamicNeighborsSet::DynamicNeighborsSet(const char *p_filePath)
{
    ...
    {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Failed read graph: size not match, expected %zu, actually %zu\n",
                     static_cast<size_t>(graphSize * sizeof(int)), static_cast<size_t>(readSize));
