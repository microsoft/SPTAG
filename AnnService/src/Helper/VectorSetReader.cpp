...
ReaderOptions::ReaderOptions(VectorValueType p_valueType, DimensionType p_dimension, VectorFileType p_fileType, std::string p_vectorDelimiter, std::uint32_t p_threadNum)
    :  m_inputValueType(p_valueType), m_dimension(p_dimension), m_inputFileType(p_fileType), m_vectorDelimiter(p_vectorDelimiter), m_threadNum(p_threadNum)
{
...
VectorSetReader::CreateInstance(std::shared_ptr<ReaderOptions> p_options)
{
    if (p_options->m_inputFileType == VectorFileType::DEFAULT) {
        ...
        return std::shared_ptr<VectorSetReader>(new TxtReader(std::move(p_options)));
    }
    else if (p_options->m_inputFileType == VectorFileType::XVEC) {
        return std::shared_ptr<VectorSetReader>(new XvecReader(std::move(p_options)));
    }
    return nullptr;
}
