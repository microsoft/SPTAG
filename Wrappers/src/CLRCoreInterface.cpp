...
{
RIterator::RIterator(std::shared_ptr<ResultIterator> result_iterator) : ManagedObject(result_iterator)
{
}
...
AnnIndex::AnnIndex(std::shared_ptr<SPTAG::VectorIndex> p_index) : ManagedObject(p_index)
{
...
AnnIndex::AnnIndex(String ^ p_algoType, String ^ p_valueType, int p_dimension)
    : ManagedObject(SPTAG::VectorIndex::CreateInstance(string_to<SPTAG::IndexAlgoType>(p_algoType),
                                                       string_to<SPTAG::VectorValueType>(p_valueType)))
{
...
void AnnIndex::SetBuildParam(String ^ p_name, String ^ p_value, String ^ p_section)
{
    if (m_Instance != nullptr)
        (*m_Instance)
            ->SetParameter(string_to_char_array(p_name), string_to_char_array(p_value),
                           string_to_char_array(p_section));
}
...
{
    if (m_Instance == nullptr || (*m_Instance)->GetIndexAlgoType() != SPTAG::IndexAlgoType::SPANN)
        return false;
...
bool AnnIndex::BuildSPANNWithMetaData(array<Byte> ^ p_meta, int p_num, bool p_withMetaIndex, bool p_normalized)
{
    ...
    if (!SPTAG::MetadataSet::GetMetadataOffsets(metaptr, p_meta->LongLength, offsets, p_num + 1, '\n'))
        return false;
    (*m_Instance)
        ->SetMetadata(new SPTAG::MemMetadataSet(
            SPTAG::ByteArray(metaptr, p_meta->LongLength, false),
    ...
    if (p_withMetaIndex)
        (*m_Instance)->BuildMetaMapping(false);
                                 ...
                                 bool p_normalized)
{
    ...
    std::shared_ptr<SPTAG::VectorSet> vectors(
        new SPTAG::BasicVectorSet(SPTAG::ByteArray(dataptr, p_data->LongLength, false), vectorType, vectorSize, p_num));
...
bool AnnIndex::Add(array<Byte> ^ p_data, int p_num, bool p_normalized)
{
    ...
    return (SPTAG::ErrorCode::Success ==
            (*m_Instance)->AddIndex(ptr, p_num, m_dimension, nullptr, false, p_normalized));
}
                               ...
                               bool p_normalized)
{
    ...
    std::shared_ptr<SPTAG::VectorSet> vectors(new SPTAG::BasicVectorSet(
        SPTAG::ByteArray(dataptr, p_data->LongLength, false), (*m_Instance)->GetVectorValueType(), m_dimension, p_num));
...
bool AnnIndex::DeleteByMetaData(array<Byte> ^ p_meta)
{
    ...
    pin_ptr<Byte> metaptr = &p_meta[0];
    return (SPTAG::ErrorCode::Success ==
            (*m_Instance)->DeleteIndex(SPTAG::ByteArray(metaptr, p_meta->LongLength, false)));
}
...
AnnIndex ^ AnnIndex::Load(String ^ p_loaderFile)
{
    ...
    AnnIndex ^ res;
    if (SPTAG::ErrorCode::Success != SPTAG::VectorIndex::LoadIndex(string_to_char_array(p_loaderFile), vecIndex) ||
        nullptr == vecIndex)
    {
...
AnnIndex ^ AnnIndex::Load(array<array<Byte> ^> ^ p_index)
{
    ...
    if (SPTAG::ErrorCode::Success !=
            SPTAG::VectorIndex::LoadIndex(std::string((char *)configptr, p_index[0]->LongLength), p_indexBlobs,
                                          vecIndex) ||
        nullptr == vecIndex)
    {
...
AnnIndex ^ AnnIndex::Merge(String ^ p_indexFilePath1, String ^ p_indexFilePath2)
{
    ...
    if (*(res->m_Instance) == nullptr || *(add->m_Instance) == nullptr ||
        SPTAG::ErrorCode::Success !=
            (*(res->m_Instance))
                ->MergeIndex(add->m_Instance->get(),
                             std::atoi((*(res->m_Instance))->GetParameter("NumberOfThreads").c_str()), nullptr))
    {
        return gcnew AnnIndex(nullptr);
    }
    return res;
...
MultiIndexScan::MultiIndexScan(array<AnnIndex ^> ^ indice, array<array<Byte> ^> ^ p_data, array<float> ^ weight,
                               int p_resultNum, bool useTimer, int termCondVal, int searchLimit)
    : ManagedObject(std::make_shared<SPTAG::MultiIndexScan>())
{
